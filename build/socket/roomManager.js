...
import consola from 'consola'; // eslint-disable-next-line no-unused-vars

import { Server, Socket } from 'socket.io'; // eslint-disable-next-line no-unused-vars
...
export default class Room {
  ...
  showPlayers() {
    ...
    this.io.to(this.roomId).emit('show-players-joined', {
      playersJoined: clients
    });
  }
  /**
  ...
  showTeams() {
    ...
    this.io.to(this.roomId).emit('show-players-teams', {
      teams
    });
  }
  /**
  ...
  beginDraft() {
    ...
    consola.info('Draft started...'); // Reset draft object to initial state
  ...
  _nextTurn() {
    this.io.to(this.roomId).emit('player-turn-end', `${this.store.clients[this.store.draft.turnNum].username} chance ended`);
  ...
  _emitTurn(currentTurnNumber) {
    ...
    consola.info(`[TURN CHANGE] ${this.store.clients[currentTurnNumber].username} is the new drafter. Turn number: ${currentTurnNumber}`);
  ...
  onDisconnect() {
    this.socker.on('disconnect', () => {
      try {
        ...
        this.showPlayers(); // Handle game reset
